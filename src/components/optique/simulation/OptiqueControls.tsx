    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input type="range" min="1" max="100" value={fiberLength} onChange={(e) => onFiberLengthChange(Number(e.target.value))}
        ...
        <div>
          ...
          <input type="range" min="0.1" max="1" step="0.01" value={attenuation} onChange={(e) => onAttenuationChange(Number(e.target.value))}
            className="w-full h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer slider-thumb" />
        </div>
      </div>
      ...
      {['épissures', 'connecteurs'].map(type => (
        <div key={type}>
          <h4 className="text-md font-semibold text-slate-200 mb-2 capitalize">{type}</h4>
          <div className="space-y-2 max-h-24 overflow-y-auto pr-2">
            {(type === 'épissures' ? splices : connectors).map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm bg-slate-700/50 p-1.5 rounded-md">
                <span>Position: <span className="font-mono text-cyan-400">{item.position}%</span></span>
                <button onClick={() => type === 'épissures' ? handleRemoveSplice(index) : handleRemoveConnector(index)}
                  className="text-red-400 hover:text-red-300 text-xs font-semibold">X</button>
              </div>
            ))}
          </div>
          <div className="pt-2">
            <div className="flex items-center space-x-2">
              <input type="range" min="0" max="100"
                ...
                className="w-full h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer slider-thumb" />
              <button onClick={type === 'épissures' ? handleAddSplice : handleAddConnector}
                className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-1 px-3 rounded-md text-sm transition-colors">+</button>
            </div>
          </div>
        </div>
      ))}
